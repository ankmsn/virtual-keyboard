import {
  lettersEng, lettersEngShift, lettersEngCaps, lettersUkr, lettersUkrShift, lettersUkrCaps,
} from './data.js'; // eslint-disable-line import/extensions
...
let lettersShift;
let lettersCaps;
const langText = document.querySelector('.top-text > p:last-child');
const langInstruction = document.querySelector('.wrapper > p');
...
function updateKeysContent(baseLetters) {
  for (let i = 0; i < baseLetters.length; i += 1) {
    cells[i].textContent = baseLetters[i];
  }
}
